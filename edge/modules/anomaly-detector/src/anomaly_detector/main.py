...
def process_record(record: dict[str, Any]) -> list[OutputMessage]:
    detected = copy_record(record)
    result = detect_anomaly(detected)
    detected["edgeDetection"] = result.as_dict()
    if result.is_anomaly:
        detected["anomalyTimestamp"] = utc_now()

    outputs: list[OutputMessage] = []
    route = detected.get("edgeRouting", {})
    if result.is_anomaly or (isinstance(route, dict) and route.get("cloudCandidate") is True):
        outputs.append(OutputMessage("telemetry", detected))
    if result.is_anomaly:
        outputs.append(OutputMessage("alerts", detected))
    return outputs
